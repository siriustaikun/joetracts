...
do
  ...
  then    
    ...
    # echo "$CODE_ID" "$file"
    echo "joed tx wasm store "cw-nfts/$file" --from $key --yes --chain-id joe-1 --gas 5000000 --broadcast-mode block --node https://rpc.justjoe.app:443/ --output json  | jq -r '.txhash'"
  fi
done

echo "============================="

files=$(ls cw-plus-contracts)
for file in $files
do
  if [[ $file == *.wasm ]]
  then    
    ...
    # CODE_ID=$(joed query tx "$STORE_HASH" --output json | jq -r '.logs[0].events[].attributes[] | select(.key == "code_id") | .value')
    # echo "$CODE_ID" "$file"
    echo "joed tx wasm store "cw-plus-contracts/$file" --from $key --yes --gas 5000000 --chain-id joe-1 --broadcast-mode block --node https://rpc.justjoe.app:443 --output json  | jq -r '.txhash'"
  fi
done